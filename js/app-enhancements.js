...
async function loadExternalAvatarArtwork(force=false){
  ...
  try{
    const [mr,fr]=await Promise.all([fetch('./male.svg?v=0492',{cache:'reload'}),fetch('./female.svg?v=0492',{cache:'reload'})]);
...
function getDayGamification(date){
  ...
  const nutrition=(appState.nutrition||[]).filter(x=>x.date===date),kcal=nutrition.reduce((s,x)=>s+Number(x.kcal||0),0),water=Number(appState.water?.[date]||0),waterTarget=getWaterTarget(date),wellness=appState.wellness?.find(x=>x.date===date),meditation=(appState.meditation||[]).filter(x=>x.date===date).reduce((s,x)=>s+Number(x.minutes||0),0),mentalPauseDone=(appState.meditation||[]).some(x=>x.date===date&&/mindfulness|body scan/i.test(String(x.type||'')))||(appState.gratitude||[]).some(x=>String(x.date||'').slice(0,10)===date),active=getActiveWorkoutPlan(),scheduled=isPlanScheduledOnDate(active,date),mealDone=mealsCompletedForDateV7(date),mealCount=new Set(nutrition.map(x=>x.meal)).size;
  ...
  steps.push({label:tr('Check-in wellness'),done:!!wellness,icon:'fa-heart-pulse'},{label:tr('Idratazione'),done:water>=waterTarget*.8,icon:'fa-glass-water'},{label:mealDone?tr('Attività completata'):`${tr('Pasti')} ${mealCount}/4`,done:mealDone,icon:'fa-utensils'},{label:tr('Pausa mentale'),done:mentalPauseDone||meditation>=5,icon:'fa-spa'});
...
function avatarSetArtworkV8(gender,{force=false}={}){
  ...
  const href=`./${file}?v=0492${force?`&thalys_avatar=${Date.now()}_${++thalysAvatarRefreshSeqV8}`:''}`;
...
// v0.37: external SVG files are the only normal avatar source.
...
/* ==========================================================
   ...
   ========================================================== */
const THALYS_APP_VERSION_V10=window.ThalysConfig?.appVersion||'0.49.2';
...
async function requestCameraStreamV18(){
  if(window.ThalysDeviceMedia?.requestCameraStream){
    return window.ThalysDeviceMedia.requestCameraStream(true);
  }
  const attempts=[
...
function stopBarcodeCameraV18(){
  ...
  if(barcodeCameraStream){
    try{
      if(window.ThalysDeviceMedia?.releaseStream)window.ThalysDeviceMedia.releaseStream(barcodeCameraStream);
      else barcodeCameraStream.getTracks().forEach(t=>t.stop());
    }catch(_){}
