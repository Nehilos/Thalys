...
async function loadExternalAvatarArtwork(force=false){
  ...
  try{
    const [mr,fr]=await Promise.all([fetch('./male.svg?v=0501',{cache:'reload'}),fetch('./female.svg?v=0501',{cache:'reload'})]);
...
function avatarSetArtworkV8(gender,{force=false}={}){
  ...
  const href=`./${file}?v=0501${force?`&thalys_avatar=${Date.now()}_${++thalysAvatarRefreshSeqV8}`:''}`;
