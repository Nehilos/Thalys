...
async function loadExternalAvatarArtwork(force=false){
  ...
  try{
    const [mr,fr]=await Promise.all([fetch('./male.svg?v=0480',{cache:'reload'}),fetch('./female.svg?v=0480',{cache:'reload'})]);
...
function avatarSetArtworkV8(gender,{force=false}={}){
  ...
  const href=`./${file}?v=0480${force?`&thalys_avatar=${Date.now()}_${++thalysAvatarRefreshSeqV8}`:''}`;
...
/* ==========================================================
   ...
   ========================================================== */
const THALYS_APP_VERSION_V10=window.ThalysConfig?.appVersion||'0.48.0';
