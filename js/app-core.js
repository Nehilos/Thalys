    ...
    window.thalysPrimaryStateReady = (async () => {
      try {
        ...
        if (record?.state) {
          ...
          // v0.37.4: recover the full user-selected profile photo from the local
          ...
          try { await window.ThalysStorage?.writePrimaryState?.(appState, { source: 'v0.37.4-profile-photo-repair' }); } catch (_) {}
