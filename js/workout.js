...
// v0.48.6: non-invasive duration estimates derived from the existing plan data.
function estimateWorkoutExercisesMinutesV0486(exercises){const arr=Array.isArray(exercises)?exercises:[];if(!arr.length)return 0;let seconds=0;arr.forEach((x,i)=>{const sets=Math.max(1,Math.round(Number(x.series)||1)),recovery=Math.max(0,Number(x.recovery)||0);seconds+=sets*45+Math.max(0,sets-1)*recovery;if(i<arr.length-1)seconds+=60;});return Math.max(1,Math.ceil(seconds/60))+15;}
function estimateWorkoutDayMinutesV0486(plan,day){const p=normalizeWorkoutPlanV7(plan),ex=p.exercises.filter(x=>x.dayOfWeek===day);return p.restDays.includes(day)?0:estimateWorkoutExercisesMinutesV0486(ex);}
function estimateWorkoutWeekMinutesV0486(plan){const p=normalizeWorkoutPlanV7(plan);return THALYS_WEEKDAYS.reduce((sum,d)=>sum+estimateWorkoutDayMinutesV0486(p,d),0);}
...
function renderPlanBuilderV7(){
  ...
  days.innerHTML=THALYS_WEEKDAYS.map(d=>`<button type="button" onclick="selectPlanBuilderDay('${d}')" class="v7-day-btn ${d===planBuilderDay?'active':''}">${tr(d)}<div class="mt-1 text-[8px] opacity-60">${planBuilderRest.has(d)?'🌙 '+tr('Riposo'):(planBuilderDraft[d]?.length||0)+' '+tr('esercizi')+' · ≈'+estimateWorkoutExercisesMinutesV0486(planBuilderDraft[d]||[])+' min'}</div></button>`).join('');
...
// v0.54.0 - Daily steps tracker (safe manual baseline, synced through app_state).
function getStepsDateV0540(){return document.getElementById('workout-date')?.value||new Date().toISOString().split('T')[0];}
function ensureStepsStateV0540(){appState.steps=(appState.steps&&typeof appState.steps==='object')?appState.steps:{};appState.stepsUpdatedAt=(appState.stepsUpdatedAt&&typeof appState.stepsUpdatedAt==='object')?appState.stepsUpdatedAt:{};appState.settings=appState.settings||{};if(!Number(appState.settings.stepTarget))appState.settings.stepTarget=8000;}
function renderDailyStepsV0540(){ensureStepsStateV0540();const d=getStepsDateV0540(),v=Math.max(0,Math.round(Number(appState.steps[d]||0))),t=Math.max(1000,Math.round(Number(appState.settings.stepTarget||8000))),pct=Math.min(100,Math.round(v/t*100));const ve=document.getElementById('workout-steps-value-v0540'),te=document.getElementById('workout-steps-target-v0540'),be=document.getElementById('workout-steps-bar-v0540');if(ve)ve.textContent=v.toLocaleString('it-IT');if(te)te.textContent=t.toLocaleString('it-IT');if(be)be.style.width=`${pct}%`;}
function changeDailyStepsV0540(delta){ensureStepsStateV0540();const d=getStepsDateV0540();appState.steps[d]=Math.max(0,Math.round(Number(appState.steps[d]||0)+Number(delta||0)));appState.stepsUpdatedAt[d]=new Date().toISOString();saveStateToLocal({source:'steps'});renderDailyStepsV0540();renderHomeDashboard?.();showToast(`${appState.steps[d].toLocaleString('it-IT')} passi`,'fa-shoe-prints');}
function editDailyStepsV0540(){ensureStepsStateV0540();const d=getStepsDateV0540(),cur=Math.round(Number(appState.steps[d]||0)),raw=prompt('Passi del giorno',String(cur));if(raw===null)return;const v=Math.max(0,Math.round(Number(String(raw).replace(',','.'))||0));appState.steps[d]=v;appState.stepsUpdatedAt[d]=new Date().toISOString();saveStateToLocal({source:'steps'});renderDailyStepsV0540();renderHomeDashboard?.();showToast('Passi aggiornati ✓','fa-shoe-prints');}
function openStepsTargetV0540(){ensureStepsStateV0540();const raw=prompt('Target passi giornaliero',String(appState.settings.stepTarget||8000));if(raw===null)return;const v=Math.max(1000,Math.min(50000,Math.round(Number(String(raw).replace(',','.'))||8000)));appState.settings.stepTarget=v;appState.settings.stepTargetUpdatedAt=new Date().toISOString();saveStateToLocal({source:'steps-target'});renderDailyStepsV0540();showToast(`Target passi: ${v.toLocaleString('it-IT')}`,'fa-bullseye');}
