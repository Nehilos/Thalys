...
function ensureStepsStateV0540(){appState.steps=(appState.steps&&typeof appState.steps==='object')?appState.steps:{};appState.settings=appState.settings||{};if(!Number(appState.settings.stepTarget))appState.settings.stepTarget=8000;}
...
function changeDailyStepsV0540(delta){ensureStepsStateV0540();const d=getStepsDateV0540();appState.steps[d]=Math.max(0,Math.round(Number(appState.steps[d]||0)+Number(delta||0)));saveStateToLocal({source:'steps'});renderDailyStepsV0540();renderHomeDashboard?.();showToast(`${appState.steps[d].toLocaleString('it-IT')} passi`,'fa-shoe-prints');}
function editDailyStepsV0540(){ensureStepsStateV0540();const d=getStepsDateV0540(),cur=Math.round(Number(appState.steps[d]||0)),raw=prompt('Passi del giorno',String(cur));if(raw===null)return;const v=Math.max(0,Math.round(Number(String(raw).replace(',','.'))||0));appState.steps[d]=v;saveStateToLocal({source:'steps'});renderDailyStepsV0540();renderHomeDashboard?.();showToast('Passi aggiornati ✓','fa-shoe-prints');}
function openStepsTargetV0540(){ensureStepsStateV0540();const raw=prompt('Target passi giornaliero',String(appState.settings.stepTarget||8000));if(raw===null)return;const v=Math.max(1000,Math.min(50000,Math.round(Number(String(raw).replace(',','.'))||8000)));appState.settings.stepTarget=v;saveStateToLocal({source:'steps-target'});renderDailyStepsV0540();showToast(`Target passi: ${v.toLocaleString('it-IT')}`,'fa-bullseye');}
