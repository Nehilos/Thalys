...
// v0.48.5: non-invasive duration estimates derived from the existing plan data.
function estimateWorkoutExercisesMinutesV0485(exercises){const arr=Array.isArray(exercises)?exercises:[];if(!arr.length)return 0;let seconds=0;arr.forEach((x,i)=>{const sets=Math.max(1,Math.round(Number(x.series)||1)),recovery=Math.max(0,Number(x.recovery)||0);seconds+=sets*45+Math.max(0,sets-1)*recovery;if(i<arr.length-1)seconds+=60;});return Math.max(1,Math.ceil(seconds/60));}
function estimateWorkoutDayMinutesV0485(plan,day){const p=normalizeWorkoutPlanV7(plan),ex=p.exercises.filter(x=>x.dayOfWeek===day);return p.restDays.includes(day)?0:estimateWorkoutExercisesMinutesV0485(ex);}
function estimateWorkoutWeekMinutesV0485(plan){const p=normalizeWorkoutPlanV7(plan);return THALYS_WEEKDAYS.reduce((sum,d)=>sum+estimateWorkoutDayMinutesV0485(p,d),0);}
...
function renderPlanBuilderV7(){
  ...
  days.innerHTML=THALYS_WEEKDAYS.map(d=>`<button type="button" onclick="selectPlanBuilderDay('${d}')" class="v7-day-btn ${d===planBuilderDay?'active':''}">${tr(d)}<div class="mt-1 text-[8px] opacity-60">${planBuilderRest.has(d)?'🌙 '+tr('Riposo'):(planBuilderDraft[d]?.length||0)+' '+tr('esercizi')+' · ≈'+estimateWorkoutExercisesMinutesV0485(planBuilderDraft[d]||[])+' min'}</div></button>`).join('');
