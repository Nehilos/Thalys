...
const DRIVE_DB_NAMES = ['thalys_manifest.json','app_state.json','nutrition_targets.json','workouts.json','workout_history.json','meal_history.json','active_plan_history.json','workout_plans.json','nutrition.json','alim_database.json','body_metrics.json','wellness_data.json','water.json','foto_index.json','foto_profilo.json','messages.json','meditation.json','gratitude.json','gratitude_deleted.json','consultations.json','ai_consults.json','lang_it.json','lang_en.json','lang_es.json','lang_pt.json','lang_ro.json'];
...
async function findDriveFolder(name,parentId=null){
    ...
    async function importPendingLegacyData(){
      ...
      try{
        ...
        await Promise.all([load('app_state.json','appState'),load('workouts.json','workouts'),load('workout_plans.json','plansPayload'),load('nutrition.json','nutrition'),load('alim_database.json','presets'),load('body_metrics.json','bodyMetrics'),load('wellness_data.json','wellness'),load('water.json','water'),load('meditation.json','meditation'),load('gratitude.json','gratitude'),load('gratitude_deleted.json','gratitudeDeleted')]);
    ...
    function databasePayloads(){
      ...
      return {
        ...
        'gratitude_deleted.json':appState.gratitudeDeleted||{},
    ...
    function mergeCloudIntoLocal(cloud){
      ...
      // v0.51.0: Pianificato follows the same clean-device rule as the dedicated
      ...
      const mergedGratitudeDeleted={...(c.gratitudeDeleted||{}),...(local.gratitudeDeleted||{})};
      result.gratitudeDeleted=mergedGratitudeDeleted;
      result.gratitude=mergeByKey(local.gratitude,c.gratitude,x=>x.id||`${x.date}|${x.text||''}`).filter(x=>!mergedGratitudeDeleted[x?.id]);
    ...
    async function loadDatabasesFromDrive(consolidate=false){
      ...
      const dedicated={
        ...
        bodyMetrics:cloud.bodyMetrics, wellness:cloud.wellness, water:cloud.water, meditation:cloud.meditation, gratitude:cloud.gratitude, gratitudeDeleted:cloud.gratitudeDeleted,
    ...
    // v0.51.0: lightweight live synchronization for Reale/Pianificato.
    ...
    async function refreshMealPlanFromDriveLive(){
      ...
      try{
        ...
        if(desktopSteps){fields.push('steps');}
        let changed=false;
        ...
        if(desktopSteps&&remote.settings&&Number(remote.settings.stepTarget)>0){
          appState.settings=appState.settings||{};
          if(Number(appState.settings.stepTarget)!==Number(remote.settings.stepTarget)){appState.settings.stepTarget=Number(remote.settings.stepTarget);changed=true;}
        }
        if(!changed)return false;
        window.appState=appState;
        persistThalysStateLocally(appState);
        if(typeof syncThalysLocalDocuments==='function')syncThalysLocalDocuments(appState);
        try{renderNutrition();}catch(_){} if(desktopSteps){try{renderDailyStepsV0540();}catch(_){}} try{renderHomeDashboard();}catch(_){} try{updateAnalyticsCharts();}catch(_){}
        setDriveStatus('ok',desktopSteps?'Dati aggiornati da Drive':'Piano alimentare aggiornato da Drive');updateManualSyncUI();
        return true;
    ...
    async function syncAfterNetworkRestore(){
      ...
      // v0.48.2: single-flight recovery. Auth, server-auth and the iOS reconnect
