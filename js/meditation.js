...
function openMindInfo(type){
    ...
    function getGratitudeEntries(){
      ...
      // One-time migration from the old local-only gratitude store.
      if(!appState.gratitude.length){try{const legacy=JSON.parse(localStorage.getItem('thalys_gratitude')||'[]');if(Array.isArray(legacy)&&legacy.length){appState.gratitude=legacy;localStorage.removeItem('thalys_gratitude');saveStateToLocal({source:'gratitude-migration'});}}catch(_){}}
      return appState.gratitude;
    }
    let gratitudeSelectMode=false;
    const gratitudeSelectedIds=new Set();
    function saveGratitudeEntry(){const ta=document.getElementById('gratitude-text'),value=ta?.value?.trim();if(!value){showToast('Scrivi prima un pensiero di gratitudine');return;}const arr=getGratitudeEntries();const now=new Date().toISOString();arr.push({id:'grat_'+Date.now(),date:now,text:value,createdAt:now,updatedAt:now});appState.gratitude=arr;saveStateToLocal({source:'gratitude'});if(ta)ta.value='';renderGratitudeHistory();renderHomeDashboard();showToast('Ricordo salvato ✓','fa-heart');}
