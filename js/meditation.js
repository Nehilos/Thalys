...
function openMindInfo(type){
    ...
    let currentGratitudeNoteId=null;
    const gratitudeSelectedIds=new Set();
    function persistGratitudeEntries(source='gratitude'){localStorage.setItem('thalys_gratitude',JSON.stringify(appState.gratitude||[]));saveStateToLocal({source});if(typeof scheduleAutosave==='function')scheduleAutosave();}
    function saveGratitudeEntry(){const ta=document.getElementById('gratitude-text'),value=ta?.value?.trim();if(!value){showToast('Scrivi prima un pensiero di gratitudine');return;}try{const arr=getGratitudeEntries();const id='grat_'+Date.now(),now=new Date().toISOString();arr.push({id,date:now,text:value,updatedAt:now});appState.gratitude=arr;appState.gratitudeDeleted=appState.gratitudeDeleted||{};delete appState.gratitudeDeleted[id];persistGratitudeEntries('gratitude');if(ta)ta.value='';renderGratitudeHistory();renderHomeDashboard();showToast('Pensiero salvato ✓','fa-heart');}catch(err){console.error('gratitude save',err);showToast('Impossibile salvare il pensiero','fa-triangle-exclamation');}}
    function deleteGratitudeEntry(id){const x=getGratitudeEntries().find(e=>e.id===id);if(!x)return;if(!confirm('Eliminare questo pensiero di gratitudine?'))return;appState.gratitude=getGratitudeEntries().filter(e=>e.id!==id);appState.gratitudeDeleted=appState.gratitudeDeleted||{};appState.gratitudeDeleted[id]=new Date().toISOString();gratitudeSelectedIds.delete(id);persistGratitudeEntries('gratitude-delete');if(currentGratitudeNoteId===id){currentGratitudeNoteId=null;closeModal('gratitude-note-modal');}renderGratitudeHistory();renderHomeDashboard();showToast('Pensiero eliminato','fa-trash');}
    function deleteCurrentGratitudeEntry(){if(currentGratitudeNoteId)deleteGratitudeEntry(currentGratitudeNoteId);}
