...
function openMindInfo(type){
    ...
    function saveGratitudeEntry(){const ta=document.getElementById('gratitude-text'),value=ta?.value?.trim();if(!value){showToast('Scrivi prima un pensiero di gratitudine');return;}try{const arr=getGratitudeEntries();arr.push({id:'grat_'+Date.now(),date:new Date().toISOString(),text:value,updatedAt:new Date().toISOString()});appState.gratitude=arr;persistGratitudeEntries('gratitude');if(ta)ta.value='';renderGratitudeHistory();renderHomeDashboard();showToast('Pensiero salvato ✓','fa-heart');}catch(err){console.error('gratitude save',err);showToast('Impossibile salvare il pensiero','fa-triangle-exclamation');}}
    function deleteGratitudeEntry(id){const x=getGratitudeEntries().find(e=>e.id===id);if(!x)return;if(!confirm('Eliminare questo pensiero di gratitudine?'))return;appState.gratitude=getGratitudeEntries().filter(e=>e.id!==id);gratitudeSelectedIds.delete(id);persistGratitudeEntries('gratitude-delete');if(currentGratitudeNoteId===id){currentGratitudeNoteId=null;closeModal('gratitude-note-modal');}renderGratitudeHistory();renderHomeDashboard();showToast('Pensiero eliminato','fa-trash');}
